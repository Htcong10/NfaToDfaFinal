...
def updateTrans(newState, alphabet, transitions, ls_eq):
    new_ls_trans = []
    new_ls_trans.append(['s0', '0', 's0'])
    new_ls_trans.append(['s0', '1', 's0'])
    for i in newState:
        if len(i) > 0:
            new_tran = []
            for j in alphabet:
